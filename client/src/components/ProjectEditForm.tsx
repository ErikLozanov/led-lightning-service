...
const ProjectEditForm = ({
    ...
    const uploadImage = async (file: File) => {
        ...
        const { error } = await supabase.storage
            .from("images")
            .upload(fileName, watermarkedFile); // or compressedFile
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const cleanExtraImages = finalExtraImages.filter(
                (url) => url !== ""
            );
    ...
    return (
        <form
            ...
            className="bg-slate-900/50 p-6 rounded-lg space-y-4 text-left border border-slate-700"
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input
                        ...
                        className="w-full bg-slate-800 border border-slate-600 rounded p-3 text-white focus:border-[#00f3ff] outline-none"
                    />
                </div>
                <div>
                    <label className="block text-sm font-bold text-gray-400 mb-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                <div className="bg-slate-800 p-4 rounded border border-slate-600">
                    <label className="block text-sm text-red-400 font-bold mb-2">
                        СМЯНА "ПРЕДИ" (Опция)
                    </label>
                    <input
                        type="file"
                        ...
                        onChange={(e) =>
                            e.target.files &&
                            setNewBeforeFile(e.target.files[0])
                        }
                        className="w-full text-sm text-gray-400"
                    />
                </div>
                <div className="bg-slate-800 p-4 rounded border border-slate-600">
                    <label className="block text-sm text-neon-blue font-bold mb-2">
                        СМЯНА "СЛЕД" (Опция)
                    </label>
                    <input
                        type="file"
                        accept="image/*"
                        onChange={(e) =>
                            e.target.files && setNewAfterFile(e.target.files[0])
                        }
                        className="w-full text-sm text-gray-400"
                    />
                </div>
            ...
            <div className="bg-slate-800 p-4 rounded border border-slate-600 mt-4">
                <label className="block text-sm text-white font-bold mb-4">
            ...
            <div className="flex gap-4 mt-6">
                <button
                    type="button"
                    onClick={onCancel}
                    className="flex-1 bg-slate-700 text-white font-bold py-3 rounded hover:bg-slate-600 transition-all"
                >
                    ОТКАЗ
                </button>
                <button
                    type="submit"
                    disabled={uploading}
                    className="flex-1 bg-neon-blue text-black font-bold py-3 rounded hover:bg-white transition-all disabled:opacity-50"
                >
