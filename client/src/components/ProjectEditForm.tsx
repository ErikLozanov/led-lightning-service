...
const ProjectEditForm = ({ project, onSuccess, onCancel }: ProjectEditFormProps) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setUploading(true);
    toast.loading("Запазване на промените...", { id: 'edit' });

    try {
      let beforeUrl = project.before_image_url;
      let afterUrl = project.after_image_url;

      if (newBeforeFile) beforeUrl = await processAndUpload(newBeforeFile);
      if (newAfterFile) afterUrl = await processAndUpload(newAfterFile);
      ...
      const finalExtraImages = await Promise.all(
        galleryItems.map(async (item) => {
          if (item.type === 'existing' && item.url) {
            ...
            return await processAndUpload(item.file); 
          }
          return '';
        })
      );
      ...
      toast.success("Промените са запазени!", { id: 'edit' });
      onSuccess();

    } catch (error) {
      console.error(error);
      toast.error("Грешка при обновяване.", { id: 'edit' });
    } finally {
      setUploading(false);
    }
  };
            ...
            {galleryItems.map((item, idx) => (
              <div key={item.id} className="relative group bg-slate-900 border border-slate-700 rounded-lg overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button type="button" onClick={() => moveGalleryItem(idx, 'left')} disabled={idx === 0} className="p-1 bg-slate-700 text-white rounded hover:bg-[#00f3ff] hover:text-black disabled:opacity-30 disabled:cursor-not-allowed"><ArrowLeft size={18} /></button>
                    <button type="button" onClick={() => moveGalleryItem(idx, 'right')} disabled={idx === galleryItems.length - 1} className="p-1 bg-slate-700 text-white rounded hover:bg-[#00f3ff] hover:text-black disabled:opacity-30 disabled:cursor-not-allowed"><ArrowRight size={18} /></button>
                    <button type="button" onClick={() => removeGalleryItem(idx)} className="p-1 bg-red-500/80 text-white rounded hover:bg-red-500"><Trash2 size={18} /></button>
                </div>
                 
                 <div className="absolute top-1 left-1 bg-black/70 text-[#00f3ff] text-xs font-bold px-2 py-0.5 rounded shadow-sm">#{idx + 1}</div>
              </div>
            ))}
          </div>
        )}
